...
exports.getUnmarkedMembers = getUnmarkedMembers;
exports.markMemberAttendance = markMemberAttendance;
exports.markVisitorAttendance = markVisitorAttendance;
exports.closeAttendance = closeAttendance;
exports.getAttendanceSummary = getAttendanceSummary;
exports.getMembersPresent = getMembersPresent;
exports.getMembersAbsent = getMembersAbsent;
exports.getVisitors = getVisitors;
exports.markAllMembersPresent = markAllMembersPresent;
exports.undoMarkAllMembers = undoMarkAllMembers;
exports.undoIndividualAttendance = undoIndividualAttendance;
exports.unmarkMemberAttendance = unmarkMemberAttendance;
exports.getWeeklyAttendanceStats = getWeeklyAttendanceStats;
exports.getMonthlyAttendanceTrends = getMonthlyAttendanceTrends;
...
async function getAttendanceStatus(req, res) {
    try {
        ...
        const isClosed = attendance?.status === Attendance_1.AttendanceStatus.CLOSED;
        return res.json({
            date,
            isClosed,
            status: attendance?.status || Attendance_1.AttendanceStatus.OPEN
        });
    }
    catch (e) {
        console.error(e);
...
async function getUnmarkedMembers(req, res) {
    try {
        const { date } = req.params;
        ...
        if (hall)
            query = query.andWhere('student.hall = :hall', { hall });
        if (level)
            query = query.andWhere('student.level = :level', { level });
        if (gender)
            query = query.andWhere('student.gender = :gender', { gender });
        ...
        const markedStudents = await attendanceRepo.find({
            where: {
                date,
                type: Attendance_1.AttendanceType.MEMBER,
                isPresent: true
            }
        });
        const markedStudentIds = markedStudents.map(a => a.studentId).filter(id => id !== null);
        // Filter out already marked students
        const unmarkedStudents = allStudents.filter(student => !markedStudentIds.includes(student.id));
...
async function markMemberAttendance(req, res) {
    try {
        const { date } = req.params;
        ...
        const existingAttendance = await repo.findOne({
            where: { date, type: Attendance_1.AttendanceType.MEMBER, studentId }
        });
        ...
        const dateStatus = await repo.findOne({
            where: { date, type: Attendance_1.AttendanceType.MEMBER }
        });
        if (dateStatus?.status === Attendance_1.AttendanceStatus.CLOSED) {
...
async function markVisitorAttendance(req, res) {
    try {
        const { date } = req.params;
        ...
        // Check if attendance is closed for this date
        const dateStatus = await repo.findOne({
            where: { date, type: Attendance_1.AttendanceType.VISITOR }
        });
        if (dateStatus?.status === Attendance_1.AttendanceStatus.CLOSED) {
            return res.status(400).json({ message: "Attendance is closed for this date" });
        }
        const attendance = repo.create({
            date,
            type: Attendance_1.AttendanceType.VISITOR,
...
async function closeAttendance(req, res) {
    try {
        ...
        await repo.update({ date, type }, { status: Attendance_1.AttendanceStatus.CLOSED });
...
async function getAttendanceSummary(req, res) {
    try {
        ...
        const members = await attendanceRepo.find({
            where: { date, type: Attendance_1.AttendanceType.MEMBER, isPresent: true }
        });
        const visitors = await attendanceRepo.find({
            where: { date, type: Attendance_1.AttendanceType.VISITOR, isPresent: true }
        });
...
async function getMembersPresent(req, res) {
    try {
        const { date } = req.params;
        const attendanceRepo = data_source_1.AppDataSource.getRepository(Attendance_1.Attendance);
...
async function getMembersAbsent(req, res) {
    try {
        const { date } = req.params;
        const attendanceRepo = data_source_1.AppDataSource.getRepository(Attendance_1.Attendance);
        const studentRepo = data_source_1.AppDataSource.getRepository(Student_1.Student);
...
async function markAllMembersPresent(req, res) {
    try {
        const { date } = req.params;
        const performerUserId = req.auth?.sub;
        const attendanceRepo = data_source_1.AppDataSource.getRepository(Attendance_1.Attendance);
        const studentRepo = data_source_1.AppDataSource.getRepository(Student_1.Student);
        const actionRepo = data_source_1.AppDataSource.getRepository(ActionLog_1.ActionLog);
        // Get all students and existing marks for the date
        const students = await studentRepo.find();
        const existing = await attendanceRepo.find({ where: { date, type: Attendance_1.AttendanceType.MEMBER } });
        const alreadyMarkedIds = new Set(existing.map(e => e.studentId).filter(Boolean));
        const createdIds = [];
...
async function undoMarkAllMembers(req, res) {
    try {
        ...
        if (action.actionType !== 'MARK_ALL_ATTENDANCE')
            return res.status(400).json({ message: 'Invalid action type' });
        if (action.undone)
            return res.status(400).json({ message: 'Action already undone' });
        const createdAttendanceIds = action.undoData?.createdAttendanceIds || [];
        if (createdAttendanceIds.length === 0)
            return res.json({ success: true, undone: 0 });
...
async function undoIndividualAttendance(req, res) {
    try {
        const { actionId } = req.params;
        const attendanceRepo = data_source_1.AppDataSource.getRepository(Attendance_1.Attendance);
        const actionRepo = data_source_1.AppDataSource.getRepository(ActionLog_1.ActionLog);
        ...
        if (!action)
            return res.status(404).json({ message: 'Action not found' });
        if (action.actionType !== 'MARK_INDIVIDUAL_ATTENDANCE')
            return res.status(400).json({ message: 'Invalid action type' });
        if (action.undone)
            return res.status(400).json({ message: 'Action already undone' });
        const attendanceId = action.undoData?.attendanceId;
        if (!attendanceId)
            return res.status(400).json({ message: 'No attendance ID found' });
...
async function unmarkMemberAttendance(req, res) {
    try {
...
function getAttendanceEmailHtml(name, date, logo) {
...
async function getWeeklyAttendanceStats(req, res) {
    try {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const dayAttendance = weeklyAttendance.filter((a) => a.date === dateStr);
            const presentCount = dayAttendance.filter((a) => a.isPresent === true).length;
            const absentCount = dayAttendance.filter((a) => a.isPresent === false).length;
            const visitorsCount = dayAttendance.filter((a) => a.type === Attendance_1.AttendanceType.VISITOR).length;
            dailyStats.push({
                date: dateStr,
        ...
        const hallStats = await attendanceRepo
            ...
            .where('attendance.date >= :startDate', { startDate: startDate.toISOString().split('T')[0] })
            .andWhere('attendance.date <= :endDate', { endDate: endDate.toISOString().split('T')[0] })
            .andWhere('attendance.type = :member', { member: Attendance_1.AttendanceType.MEMBER })
...
async function getMonthlyAttendanceTrends(req, res) {
    try {
        ...
        for (let i = parseInt(months.toString()) - 1; i >= 0; i--) {
            ...
            const monthlyAttendance = await attendanceRepo
                ...
                .andWhere('attendance.date <= :endDate', { endDate: endDate.toISOString().split('T')[0] })
                .getMany();
            const presentCount = monthlyAttendance.filter((a) => a.isPresent === true).length;
            const visitorsCount = monthlyAttendance.filter((a) => a.type === Attendance_1.AttendanceType.VISITOR).length;
...
function getVisitorAttendanceEmailHtml(name, date, logo) {
    const formattedDate = new Date(date).toLocaleDateString('en-US', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
