...
exports.getExportFilters = getExportFilters;
...
async function exportStudents(req, res) {
    try {
        ...
        const { hall, level, gender, role, status, programDuration, admissionYear } = filters;
        const { startDate, endDate } = dateRange || {};
        const { personalInfo = true, contactInfo = true, attendance = false } = includeOptions || {};
        ...
        if (role)
            query = query.andWhere('student.role = :role', { role });
        if (status) {
            // For now, all students are considered "Active"
            // You can modify this based on your status logic
        }
        if (programDuration)
            query = query.andWhere('student.programDurationYears = :programDuration', { programDuration: parseInt(programDuration) });
        if (admissionYear) {
...
async function exportToPDF(res, data, filename) {
    try {
        const doc = new jspdf_1.default('landscape');
        // Add title
        doc.setFontSize(16);
...
async function exportToCSV(res, data, filename) {
    ...
    const csvContent = [
        headers.join(','),
        ...data.map(row => Object.values(row).map(value => typeof value === 'string' && value.includes(',')
            ? `"${value}"`
            : value).join(','))
    ].join('\n');
    // Set response headers
...
async function getExportFilters(req, res) {
    try {
        const studentRepo = data_source_1.AppDataSource.getRepository(Student_1.Student);
