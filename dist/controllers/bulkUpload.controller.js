...
exports.undoBulkUpload = undoBulkUpload;
...
function normalizeLevel(level) {
    ...
    if (levelStr.includes('100'))
        return 'L100';
    if (levelStr.includes('200'))
        return 'L200';
    if (levelStr.includes('300'))
        return 'L300';
    if (levelStr.includes('400'))
        return 'L400';
    if (levelStr.includes('500'))
        return 'L500';
    if (levelStr.includes('600'))
        return 'L600';
...
function validateStudentData(row, rowIndex) {
    ...
    if (!row.hall?.toString().trim())
        errors.push('Hall/Hostel is required');
...
async function generateStudentCode(repo) {
    ...
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                const savedStudent = await repo.save(student);
...
async function undoBulkUpload(req, res) {
    try {
        ...
        if (action.actionType !== 'BULK_UPLOAD_STUDENTS')
            return res.status(400).json({ message: 'Action type not undoable by this endpoint' });
        if (action.undone)
            return res.status(400).json({ message: 'Action already undone' });
        const createdIds = action.metadata?.createdStudentIds || [];
        ...
        for (let i = 0; i < createdIds.length; i += chunkSize) {
            ...
            await studentRepo.delete({ id: (0, typeorm_1.In)(batch) });
        }
        action.undone = true;
